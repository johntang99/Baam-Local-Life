...
const MATCH_STYLES = {
  high: { bg: 'border-green-300 bg-green-50', badge: 'bg-green-500 text-white', label: '高度匹配' },
  medium: { bg: 'border-amber-300 bg-amber-50', badge: 'bg-amber-500 text-white', label: '可能匹配' },
  low: { bg: 'border-blue-300 bg-blue-50', badge: 'bg-blue-500 text-white', label: '值得探索' },
};
...
export function ScreenerClient() {
  ...
  if (loading) {
    return (
      <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-2">{convert('AI正在分析你的情况...')}</h3>
        <p className="text-sm text-gray-500">{convert('基于你提供的信息，我们的AI助手正在评估可能适合你的签证类别')}</p>
      </div>
    );
  }
    ...
    return (
      <div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6">
          <div className="text-xs font-semibold text-gray-500 mb-2">{convert('你的基本信息：')}</div>
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-1">{convert('你的签证资格评估结果')}</h2>
          <p className="text-sm text-gray-500">{convert('基于你的个人情况，以下是可能适合你的签证类别')}</p>
        </div>
              ...
              <div key={i} className={cn('border-2 rounded-2xl p-5 sm:p-6', style.bg)}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                  <div className="bg-white/60 rounded-lg p-3">
                    <div className="text-[10px] text-gray-500 mb-0.5">{convert('处理时间')}</div>
                    <div className="text-xs font-semibold text-gray-900">{cat.processingTime}</div>
                  </div>
                  <div className="bg-white/60 rounded-lg p-3">
                    <div className="text-[10px] text-gray-500 mb-0.5">{convert('雇主要求')}</div>
                    <div className="text-xs font-semibold text-gray-900">{cat.employerRequired ? convert('需要雇主') : convert('不需要')}</div>
                  </div>
                  {cat.requirements?.[0] && (
                    <div className="bg-white/60 rounded-lg p-3">
                      <div className="text-[10px] text-gray-500 mb-0.5">{convert('关键条件')}</div>
        ...
        {result.notes?.length > 0 && (
          <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-6">
            <h3 className="text-sm font-bold text-gray-900 mb-3">{convert('重要提示')}</h3>
        ...
        <div className="bg-gradient-to-br from-primary to-orange-600 rounded-2xl p-6 sm:p-8 text-center text-white mb-6">
          <h3 className="text-lg font-bold mb-2">{convert('想了解哪个方案最适合你？')}</h3>
          <p className="text-sm text-white/80 mb-5">{convert('免费咨询移民律师 — 30分钟一对一评估，帮你制定最佳移民策略')}</p>
          <button className="px-8 py-3 bg-white text-primary font-bold rounded-xl hover:bg-gray-50 transition">
            {convert('预约免费咨询')}
          </button>
          <p className="text-xs text-white/60 mt-3">{convert('已有 500+ 华人通过Baam找到合适的移民律师')}</p>
        </div>
  ...
  return (
    <div>
      ...
      {step > 1 && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6">
          <div className="text-xs font-semibold text-gray-500 mb-2">{convert('你填写的信息：')}</div>
          <div className="flex flex-wrap gap-2">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 sm:p-6">
        ...
        {step === 1 && (
          <>
            <h2 className="text-lg font-bold text-gray-900 mb-1">{convert('你的目标是什么？')}</h2>
            <p className="text-sm text-gray-500 mb-4">{convert('选择最符合你当前需求的选项')}</p>
            <div className="space-y-2">
              {GOALS.map((g) => (
        ...
        {step === 2 && (
          <>
            <h2 className="text-lg font-bold text-gray-900 mb-4">{convert('你的国籍和出生地')}</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{convert('国籍')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{convert('出生地（国家）')}</label>
                <input type="text" value={form.birthCountry} onChange={(e) => updateForm('birthCountry', e.target.value)}
                  className="w-full h-11 px-4 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-primary outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{convert('当前位置')}</label>
        ...
        {step === 3 && (
          <>
            <h2 className="text-lg font-bold text-gray-900 mb-4">{convert('你目前的身份')}</h2>
            <div className="space-y-2">
              {STATUSES.map((s) => (
        ...
        {step === 4 && (
          <>
            <h2 className="text-lg font-bold text-gray-900 mb-4">{convert('工作和职业信息')}</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{convert('职业/行业')}</label>
                <input type="text" value={form.occupation} onChange={(e) => updateForm('occupation', e.target.value)}
                  placeholder={convert('如：软件工程师、会计、厨师')}
                  className="w-full h-11 px-4 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-primary outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{convert('工作经验（年）')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{convert('雇主是否愿意担保签证/绿卡？')}</label>
        ...
        {step === 5 && (
          <>
            <h2 className="text-lg font-bold text-gray-900 mb-4">{convert('家庭关系')}</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{convert('你有美国公民的直系亲属吗？')}</label>
              ...
              {form.hasUSCitizenFamily && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">{convert('关系')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{convert('你有绿卡持有者的直系亲属吗？')}</label>
                <div className="flex gap-3">
        ...
        {step === 6 && (
          <>
            <h2 className="text-lg font-bold text-gray-900 mb-4">{convert('教育背景')}</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{convert('最高学历')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{convert('专业领域')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{convert('你在专业领域有突出成就吗？')}</label>
                <p className="text-xs text-gray-400 mb-2">{convert('如：发表论文、专利、行业获奖、媒体报道等')}</p>
                <div className="flex gap-3">
