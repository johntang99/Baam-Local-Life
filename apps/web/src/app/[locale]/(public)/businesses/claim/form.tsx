...
export function ClaimForm({ categories }: ClaimFormProps) {
  ...
  if (success) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-lg fw-bold mb-2">申请已提交！</h3>
  ...
  const inputClass = 'w-full h-11 px-4 border border-border r-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none bg-bg-card';
  ...
  return (
    <form action={handleSubmit} className="space-y-5">
      ...
      <div>
        <label className="block text-sm fw-medium mb-1.5">商家名称 *</label>
        <input name="display_name" type="text" placeholder="请输入商家名称" required className={inputClass} />
      </div>

      <div>
        <label className="block text-sm fw-medium mb-1.5">商家类别 *</label>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm fw-medium mb-1.5">联系电话 *</label>
          <input name="phone" type="tel" placeholder="(xxx) xxx-xxxx" required className={inputClass} />
        </div>
        <div>
          <label className="block text-sm fw-medium mb-1.5">电子邮箱</label>
          <input name="email" type="email" placeholder="your@email.com" className={inputClass} />
        </div>
      </div>

      <div>
        <label className="block text-sm fw-medium mb-1.5">商家地址</label>
        <input name="address" type="text" placeholder="例：136-20 Roosevelt Ave, Flushing, NY 11354" className={inputClass} />
      </div>

      <div>
        <label className="block text-sm fw-medium mb-1.5">网站</label>
        <input name="website" type="url" placeholder="https://..." className={inputClass} />
      </div>

      <div>
        <label className="block text-sm fw-medium mb-1.5">商家简介</label>
        <textarea
          name="description"
          rows={3}
          placeholder="简要介绍您的商家服务..."
          className="w-full px-4 py-3 border border-border r-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none bg-bg-card resize-none"
        />
      </div>

      <button type="submit" disabled={loading} className={cn(buttonVariants(), 'w-full h-12 text-base fw-semibold disabled:opacity-50')}>
        {loading ? '提交中...' : '提交入驻申请'}
      </button>
    </form>
  );
