...
// Emoji mapping for logo placeholder
...
// Gradient colors for logo placeholder
const categoryGradients: Record<string, string> = {
  '餐饮美食': 'from-yellow-200 to-yellow-300',
  '医疗健康': 'from-blue-200 to-blue-400',
  '法律移民': 'from-red-200 to-red-400',
  '地产保险': 'from-green-200 to-green-300',
  '教育培训': 'from-pink-200 to-pink-300',
  '装修家居': 'from-cyan-200 to-cyan-300',
  '汽车服务': 'from-slate-200 to-slate-300',
  '财税服务': 'from-green-200 to-green-300',
...
export default async function BusinessListPage() {
  ...
  // Fetch active businesses, featured first
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-text-muted mt-1">
                共 {businesses.length} 家商家 · 本地华人商家信息
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="bg-bg-card border-b border-border sticky top-16 z-40">
        <div className="max-w-7xl mx-auto px-4 py-4 space-y-3">
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide -mx-4 px-4">
            <button className="flex-shrink-0 px-4 py-2 text-sm font-medium rounded-full bg-primary text-text-inverse">
              全部
            </button>
            {categories.map((cat) => (
              <button
                key={cat.id}
                className="flex-shrink-0 px-4 py-2 text-sm font-medium rounded-full bg-border-light text-text-secondary hover:bg-primary/10 hover:text-primary transition"
              >
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <div className="sm:ml-auto">
              <select className="h-9 px-3 pr-8 text-sm border border-border rounded-lg bg-bg-card text-text-primary focus:ring-2 focus:ring-primary focus:border-primary outline-none cursor-pointer">
                <option>推荐</option>
                <option>评分最高</option>
                <option>最近更新</option>
              </select>
                ...
                {featured.map((biz) => {
                  ...
                  return (
                    <Link
                      ...
                      className="card-featured block relative overflow-hidden"
                    >
                      <div className="absolute top-0 left-0 bg-primary text-text-inverse text-xs font-bold px-3 py-1 rounded-br-lg">
                        推荐
                      </div>
                      <div className="p-5 sm:p-6">
                        <div className="flex items-start gap-4">
                          <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-gradient-to-br ${gradient} flex-shrink-0 flex items-center justify-center text-2xl sm:text-3xl`}>
                            {emoji}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <h3 className="font-bold text-base sm:text-lg truncate">
                                {biz.name_zh || biz.name}
                              </h3>
                              {biz.is_verified && (
                            ...
                            <div className="flex items-center justify-between">
                              <p className="text-xs text-text-muted">
                                {biz.region || biz.city || ''}
                                {biz.address_short ? ` · ${biz.address_short}` : ''}
                              </p>
                              <span className="text-xs font-medium text-text-inverse bg-primary px-4 py-1.5 rounded-lg">
                                联系
                              </span>
                            </div>
                          </div>
              ...
              {standard.map((biz) => {
                ...
                return (
                  ...
                  >
                    <div className="p-5">
                      <div className="flex items-start gap-4">
                        <div className={`w-14 h-14 rounded-lg bg-gradient-to-br ${gradient} flex-shrink-0 flex items-center justify-center text-xl`}>
                          {emoji}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
