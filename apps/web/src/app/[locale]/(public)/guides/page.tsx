...
export default async function GuidesListPage() {
  ...
  if (categoryGroups.length === 0 && articles.length > 5) {
    ...
    for (let i = 0; i < Math.min(3, Math.ceil(remaining.length / 4)); i++) {
      categoryGroups.push({
        category: { name_zh: ['热门指南', '实用攻略', '精选推荐'][i], icon: ['🔥', '📚', '⭐'][i] },
      ...
      <div className="bg-bg-card border-b border-border sticky top-16 z-40">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex gap-1 overflow-x-auto py-3" style={{ WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none' }}>
            <Link
              href="/guides"
              className="flex-shrink-0 px-4 py-2 text-sm font-medium bg-primary text-text-inverse rounded-full"
            >
              全部
            </Link>
            {categories.map((cat) => (
          ...
          <div className="flex-1 min-w-0 space-y-10">
            ...
            {error ? (
              ...
              <div className="py-12 text-center">
                ...
                <p className="text-text-secondary">暂无指南内容</p>
                <p className="text-text-muted text-sm mt-1">指南将在这里显示</p>
              </div>
            ) : (
              <>
                  ...
                  <div className="grid lg:grid-cols-5 gap-5">
                    ...
                    {featuredGuide && (
                      <Link
                        ...
                        className="lg:col-span-3 card card-featured cursor-pointer group overflow-hidden block"
                      >
                        <div className={`relative h-56 sm:h-72 bg-gradient-to-br ${getGradient(0)} flex items-end`}>
                          ...
                          <div className="relative p-5 sm:p-6 w-full">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className="px-2.5 py-0.5 bg-white/20 text-white text-xs font-medium rounded-full backdrop-blur-sm">编辑精选</span>
                            </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2 mb-3">
                            ...
                            {featuredGuide.read_time_minutes && (
                              <>
                                ...
                                <span className="text-xs text-text-muted">{featuredGuide.read_time_minutes}分钟阅读</span>
                              </>
                            )}
                          </div>
                    ...
                    <div className="lg:col-span-2 space-y-4">
                      ...
                      {recentGuides.map((guide, idx) => {
                        ...
                        return (
                          <Link key={guide.id} href={`/guides/${guide.slug}`} className="card p-4 flex gap-4 cursor-pointer group block">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5 mb-1">
                                <span className={`badge ${vertical.className} text-xs`}>{vertical.label}</span>
                              ...
                              <p className="text-xs text-text-muted mt-1">
                                ...
                                {guide.read_time_minutes ? ` · ${guide.read_time_minutes}分钟阅读` : ''}
                              </p>
                            </div>
                          </Link>
                        );
                  ...
                  <section key={group.category.id || groupIdx}>
                    <div className="flex items-center justify-between mb-5">
                      <h2 className="text-xl font-bold flex items-center gap-2">
                        ...
                        {group.category.name_zh || group.category.name_en || group.category.name}
                      </h2>
                      <span className="text-sm text-primary font-medium hover:underline cursor-pointer">查看全部 →</span>
                    </div>
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
                      {group.guides.map((guide, idx) => {
                        const vertical = verticalConfig[guide.content_vertical] || { label: '指南', className: 'badge-gray' };
                        return (
                          <Link key={guide.id} href={`/guides/${guide.slug}`} className="card cursor-pointer group block">
                              ...
                              {guide.audience_tags && Array.isArray(guide.audience_tags) && (
                                <div className="flex flex-wrap gap-1.5 mt-2">
                                  {guide.audience_tags.slice(0, 2).map((tag: string) => (
                                    <span key={tag} className="text-xs bg-accent-green-light text-green-700 px-1.5 py-0.5 rounded">{tag}</span>
                                  ))}
                                </div>
                              )}
                              {guide.read_time_minutes && (
                                <p className="text-xs text-text-muted mt-2">{guide.read_time_minutes}分钟阅读</p>
                              )}
                            </div>
                          </Link>
                        );
            ...
            <div className="sticky top-36 space-y-6">
              ...
              <div className="bg-bg-card border border-border rounded-xl p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {['中文家庭医生', '报税服务', '驾照路考', '学区排名', '白卡申请', '租房攻略', '搬家清单', 'ITIN申请'].map((term) => (
                    <span
                      key={term}
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      <span className="text-xs text-text-muted">4.7 (94评价)</span>
                    </div>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  <span className="text-xs bg-accent-blue-light text-secondary-dark px-2 py-0.5 rounded-full">中文服务</span>
                  <span className="text-xs bg-accent-red-light text-accent-red px-2 py-0.5 rounded-full">报税季热门</span>
                  <span className="text-xs bg-accent-green-light text-green-700 px-2 py-0.5 rounded-full">免费咨询</span>
                </div>
                <div className="bg-primary-50 border border-primary-200 rounded-lg p-3 mb-3">
                  <p className="text-xs text-primary-700"><strong>报税季特惠：</strong>新客户首次报税享8折优惠。个人报税低至$88起。</p>
                </div>
                <p className="text-xs text-text-muted mb-3">136-40 39th Ave, Flushing, NY 11354</p>
                <Link href="/businesses" className="block text-center btn btn-primary py-2.5 text-sm">了解详情</Link>
              </div>
