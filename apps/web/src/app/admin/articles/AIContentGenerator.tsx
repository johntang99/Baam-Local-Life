...
const styles = [
  { value: '实用指导', label: '实用指导', desc: '怎么做、步骤建议' },
  { value: '新闻报道', label: '新闻报道', desc: '时事、政策、事件' },
  { value: '深度分析', label: '深度分析', desc: '来龙去脉、影响解读' },
  { value: '生活分享', label: '生活分享', desc: '亲历、体验、故事' },
];

const tones = [
  { value: '亲切友好', label: '亲切友好' },
...
export function AIContentGenerator({ onGenerated }: AIContentGeneratorProps) {
  ...
  const [region, setRegion] = useState('纽约');
  const [category, setCategory] = useState('指南');
  const [style, setStyle] = useState('实用指导');
  const [tone, setTone] = useState('亲切友好');
  const [audience, setAudience] = useState('所有人');
        ...
        <div className="flex items-center gap-3">
          {lastPrompt && (
            ...
            >
              ...
              查看Prompt ({lastModel} · {lastTokens} tokens)
            </button>
          )}
          ...
          {mode === 'generate' ? (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">主题 / 标题 *</label>
                  <input
                    type="text"
                    value={topic}
                    onChange={(e) => setTopic(e.target.value)}
                    placeholder="例：法拉盛中文家庭医生推荐"
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="逗号分隔，如：家庭医生, 法拉盛, 中文服务"
                    className={inputClass}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3">
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">地区</label>
                  <select value={region} onChange={(e) => setRegion(e.target.value)} className={selectClass}>
                    <option value="纽约">纽约</option>
                    <option value="法拉盛">法拉盛</option>
                ...
                <div>
                  ...
                  <select value={category} onChange={(e) => setCategory(e.target.value)} className={selectClass}>
                    ...
                    <option value="对比评测">对比评测</option>
                    <option value="清单">清单</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">写作风格</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">语气</label>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">目标受众</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">参考来源 URL</label>
                  <input
                    type="url"
                    value={sourceUrl}
                ...
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">编辑备注</label>
                  <input
                    type="text"
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    placeholder="给AI的额外指示（可选）"
                    className={inputClass}
                  />
                </div>
              </div>
            ...
            <div className="space-y-3">
              {/* Source Content */}
              <div>
                <label className="block text-xs font-medium text-text-secondary mb-1">原文内容 *</label>
                <textarea
                  value={sourceContent}
              ...
              <div className="grid grid-cols-4 gap-3">
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">改写风格</label>
                  <select value={style} onChange={(e) => setStyle(e.target.value)} className={selectClass}>
                    {styles.map((s) => (
                      <option key={s.value} value={s.value}>{s.label}</option>
                    ))}
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  {mode === 'generate' ? 'AI 生成完整文章' : 'AI 改写文章'}
                </>
              )}
            </button>
            {loading && (
              <span className="text-xs text-text-muted">将自动填充标题、正文、摘要、FAQ、SEO...</span>
            )}
          </div>
        </div>
      )}
