...
export default async function DentalTreatmentsPage() {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold">治疗方案管理</h1>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b">
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">最低价</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">最高价</th>
          ...
          <tbody className="divide-y">
            {data.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-600">{row.cost_min != null ? `$${row.cost_min}` : '—'}</td>
                <td className="px-4 py-3 text-gray-600">{row.cost_max != null ? `$${row.cost_max}` : '—'}</td>
