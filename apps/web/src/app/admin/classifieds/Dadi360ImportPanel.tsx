...
import { runDadi360Import, type DadiImportTarget } from './actions';

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type AnyRow = Record<string, any>;

const targets: Array<{ key: DadiImportTarget; label: string; hint: string }> = [
  { key: 'housing_rent', label: '抓取租房', hint: '论坛 92' },
  { key: 'jobs', label: '抓取招聘', hint: '论坛 29' },
  { key: 'secondhand', label: '抓取二手', hint: '论坛 23' },
  { key: 'all', label: '一键抓取全部', hint: '租房+招聘+二手' },
];

export default function Dadi360ImportPanel() {
  const [isPending, startTransition] = useTransition();
  const [status, setStatus] = useState<string>('仅抓取最新新帖；历史已抓取内容会自动跳过，并自动整理为统一结构。');
  const [results, setResults] = useState<AnyRow[]>([]);

  const handleRun = (target: DadiImportTarget) => {
    setStatus('抓取中，请稍候...');
    setResults([]);
    startTransition(async () => {
      const res = await runDadi360Import(target);
  ...
  return (
    <div className="mb-6 rounded-xl border border-border bg-bg-card p-4">
      <div className="flex flex-col gap-1 mb-3">
        <h2 className="text-sm font-semibold">Dadi360 自动抓取</h2>
